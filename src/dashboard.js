...
// WS open to all connections — PIN on REST endpoints only.
...
// ── FULL STATE — every field the frontend reads ───────────────────────────────
function fullState() {
  const H = hot()
  ...
  const flashBase   = H[2]
  const flashReserve= H[3]
  const flashTotal  = flashBase + flashReserve
  const uptime      = H[8] | 0
  const execCount   = H[7] | 0
  const memMB       = process.memoryUsage().heapUsed / 1024 / 1024 | 0
  const dailyRevenue= H[1]

  // throughput: flash × cycles × rate — what the frontend reads as s.throughput
  const throughput  = flashTotal * 8_000_000 * 0.00045

  // treasury yield — daily figure, renamed from treasuryYield to match frontend
  const treasuryYieldDay = H[5] > 0
    ? H[5] * (0.20 * 0.065 + 0.50 * 0.042 + 0.30 * 0.0335) / 365
    : 0
  ...
  let dbOk = true
  try { getExecutions(1); dbOk = true } catch { dbOk = false }
  ...
  return {
    ...
    target,
    dailyRevenue,
    revPct:           target > 0 ? Math.min(dailyRevenue / target * 100, 100) : 0,
    // ── Flash
    flashBase,
    flashReserve,
    flashTotal,
    // ── Throughput — field the frontend reads
    throughput,
    // ── Signals
    crashSignal:      H[4],
    // ── Treasury
    treasury:         H[5],
    treasuryYieldDay, // ← renamed: frontend reads s.treasuryYieldDay
    // ── Amplifier
    amplifierBonus:   H[10],
    totalAmplified:   H[11],
    // ── Ops
    ...
    dbOk,             // ← added: frontend reads s.dbOk
  }
}
...
app.get('/', (_, res) => {
  ...
  existsSync(p) ? res.sendFile(p) : res.status(404).send('daybreak.html not found')
})
app.get('/eye', (_, res) => {
  const p = path.join(__dir, '../dashboard/eye.html')
  existsSync(p) ? res.sendFile(p) : res.status(404).send('eye.html not found')
})
...
app.get('/api/bridges',    auth, (_, res) => {
  ...
  for (const [k, v] of Object.entries(process.env)) {
    if (k.match(/^[A-Z][A-Z0-9]+_SECRET_KEY$/) && v)
      bridges.push(k.replace('_SECRET_KEY','').toLowerCase())
  }
  res.json({ bridges })
})
  ...
  try {
    ...
    try {
      recordTransfer({
        ...
        bridge,
        recipient: params.phone || params.accountNumber || params.address || '',
        status:    'submitted',
...
app.post('/api/chat',      auth, async (req, res) => {
  ...
  try {
    const response = await new Promise((resolve, reject) => {
      ...
      const handler = msg => {
        if (msg?.type === 'chatReply' && msg.id === id) {
          SOVEREIGN_W.off('message', handler)
    ...
    const H = hot()
    res.json({
      response: `SOVEREIGN P${H?H[0]:5} | uptime ${H?H[8]|0:0}s | type status for report`,
      ts: Date.now(),
    })
  }
})
...
// ── WEBSOCKET — NO PIN, OPEN TO ALL CONNECTIONS ───────────────────────────────
// PIN was removed from WS. This was the source of every "connecting forever"
// failure — PIN mismatch on upgrade silently rejected the socket.
// Security: Railway HTTPS protects transport. HTML PIN gate protects the UI.
// The WS is a read-only data pipe. No sensitive actions go through WS.
...
export function startDashboard(SAB, chains, sovereignWorker) {
  ...
  srv.listen(PORT, () => {
    ...
    console.log(`[DASHBOARD] WS open — no PIN on upgrade (PIN on REST only)`)
